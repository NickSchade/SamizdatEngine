...
public static class ErosionBuilder {
    ...
    public static void ThermalErosion(float[,] Elevation, float talusAngle = 0.5f, int iterations = 1, Benchmark bench = null)
    {
        ...
        for (int i = 0; i < iterations; i++)
        {
            for (int x = 0; x < xDim; x++)
            {
                for (int y = 0; y < yDim; y++)
                {
                    ThermalWorkhorse(Elevation, new Loc(x,y), talusAngle);
                }
            }
    ...
    public static void ThermalWorkhorse(float[,] Elevation, Loc l, float talusAngle)
    {
        Dictionary<Loc, float> n = MapUtil.GetValidNeighbors(Elevation, l); // Thermal Erosion
        float di;
        float dtotal = 0f;
        float dmax = float.NegativeInfinity;
        foreach (KeyValuePair<Loc, float> kvp in n)
        {

...
public class RainBuilder  {
    ...
    private void BuildRainByWindWorkhorse(ref List<Wind> winds, WindSpawn windSpawn, WindRainType rainType, WindEvaporationType evapType, WindTurnType windTurnType)
    {
        ...
        {
            WindVectorsList[w.approxLoc.x, w.approxLoc.y].Add(new Vec(w.velocity.x, w.velocity.y));
        }
    ...
    private void CombineWinds(ref List<Wind> winds, WindSpawn ws, bool addNewWinds = false)
    {
        ...
        foreach (Wind w in winds)
        {
            stackedWindsList[w.approxLoc.x, w.approxLoc.y].Add(w);
        }
        List<Wind> newWinds = new List<Wind>();
        for (int x = 0; x < xDim; x++)
        {
            for (int y = 0; y < yDim; y++)
            {
                ...
                {
                    newWinds.Add(CombineWindsAtLocation(stackedWindsList[x, y]));
                }
            }
        }
        winds = newWinds;
    }
    private Wind CombineWindsAtLocation(List<Wind> windsAtLocation)
    {
        ...
        foreach (Wind w in windsAtLocation)
        {
            xl.Add(w.actualLoc.x);
            yl.Add(w.actualLoc.y);
            xv.Add(w.velocity.x);
            yv.Add(w.velocity.y);
            water.Add(w.Water);
        }
...
{
    public Vec velocity;
    public Vec actualLoc;
    public Loc approxLoc;
    ...
    public Wind(float _x, float _y, float[,] Elevation, WindSpawn ws)
    {
        ...
        deltaHeight = 0f;
        deltaTemp = 0f;
        actualLoc = new Vec(_x, _y);
        approxLoc = new Loc((int)_x, (int)_y);
        WindOnMap = true;
        xDim = Elevation.GetLength(0);
        yDim = Elevation.GetLength(1);
    ...
    {
        deltaHeight = Elevation[approxLoc.x, approxLoc.y];
        deltaTemp = Temperature[approxLoc.x, approxLoc.y];
        actualLoc = new Vec(actualLoc.x + velocity.x, actualLoc.y + velocity.y);
        approxLoc = new Loc((int)actualLoc.x, (int)actualLoc.y);
        WindOnMap = true;
        WindOnMap = (actualLoc.x < 0 || actualLoc.x >= xDim) ? false : WindOnMap;
        WindOnMap = (approxLoc.x < 0 || approxLoc.x >= xDim) ? false : WindOnMap;
        WindOnMap = (actualLoc.y < 0 || actualLoc.y >= yDim) ? false : WindOnMap;
        WindOnMap = (approxLoc.y < 0 || approxLoc.y >= yDim) ? false : WindOnMap;
        if (WindOnMap)
        {
            deltaHeight = Elevation[approxLoc.x, approxLoc.y] - deltaHeight;
            deltaTemp = Temperature[approxLoc.x, approxLoc.y] - deltaTemp;
        }
    }
    public void Rain(WindRainType rt, ref float[,] Rain, float [,] Elevation, float seaLevel)
    {
        ...
        {
            if (deltaHeight > 0f && Elevation[approxLoc.x,approxLoc.y] > seaLevel)
            {
        ...
        {
            if (deltaTemp < -0f && Elevation[approxLoc.x, approxLoc.y] > seaLevel)
            {
                float rainAmount = 0.1f * Water;
                RainWorkhorse(ref Rain, rainAmount);
            }
    ...
    {
        Water -= rainAmount;
        Rain[approxLoc.x, approxLoc.y] += Water;
    }
    public void Evaporate(WindEvaporationType et, float[,] Elevation, float[,] Temperature, float seaLevel)
    {
        ...
        {
            if (Elevation[approxLoc.x,approxLoc.y] < seaLevel)
            {
                Water += 1f;
            }
        }
        else if (et == WindEvaporationType.WaterAndHeat)
        {
            if (Elevation[approxLoc.x, approxLoc.y] < seaLevel)
            {
                Water += Temperature[approxLoc.x,approxLoc.y];
            }
        }
    }
    public void TurnWind(WindTurnType wt, float[,] Elevation, float seaLevel)
    {
        ...
        else if (wt == WindTurnType.TerrainBased)
        {
            if (Elevation[approxLoc.x, approxLoc.y] > seaLevel)
            {
                ...
                Loc lLoc = new Loc((int)(Math.Max(1,actualLoc.x + lVec.x)), (int)Math.Max(1,actualLoc.y + lVec.y));
                Loc rLoc = new Loc((int)(Math.Max(1,actualLoc.x + rVec.x)), (int)Math.Max(1,actualLoc.y + rVec.y));
                try
                {
                    float fDiff = Elevation[lLoc.x, lLoc.y] - Elevation[rLoc.x, rLoc.y];
                    velocity.x += lLoc.x * fDiff;
                    velocity.y += lLoc.y * fDiff;
                }
                catch
                {
                    // ¯\_(ツ)_/¯
    ...
    {
        float yC = (yDim - 1) / 2f;
        float EquatorToPolePosition = Math.Abs(approxLoc.y - yC) / yC;
        ...
        //Debug.Log("Wind at " + approxLoc.y + "("+EquatorToPolePosition+") is "+xDir+","+yDir);
